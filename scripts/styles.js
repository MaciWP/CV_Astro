...
const MAIN_CSS_FILES = ["global.css", "main.css", "combined.css"]; // Archivos principales que incluyen imports y fixes combinados
...
async function processImports(filePath, processedFiles = new Set()) {
  ...
  try {
    ...
    while ((match = importRegex.exec(content)) !== null) {
      ...
      const importedContent = await processImports(
        resolvedPath,
        processedFiles,
      );
...
async function processAndCopyCSS(fileName, optimize = false) {
  ...
  try {
    ...
    console.log(
      `✓ Processed and copied: ${fileName}${optimize ? " (optimized)" : ""}`,
    );
  } catch (error) {
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  ...
  processStyles(optimize).catch((error) => {
    console.error("Script failed:", error);
    process.exit(1);
  });
}

export { processStyles, processAndCopyCSS, optimizeCSS };
