...
async function checkTranslations() {
    ...
    try {
        ...
        if (Object.keys(translations).length === 0) {
            console.error('❌ No translation files found. Verify the locales directory structure.');
            return;
        }

        // Use English as the reference language if available, otherwise use the first available
        const referenceLanguage = translations.en ? 'en' : Object.keys(translations)[0];
        ...
        console.log(`Found ${referenceKeys.length} translation keys in ${referenceLanguage}\n`);
