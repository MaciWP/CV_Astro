...
async function processImage(filePath) {
  try {
    ...
    const placeholderPath = path.join(
      DEST_DIR,
      `${filenameWithoutExt}-placeholder.jpg`,
    );
    ...
    for (const size of imageSizes) {
      ...
      console.log(
        `  ✓ Generated WebP (${size}px): ${path.basename(webpOutput)}`,
      );
    }

    // Generate fallback JPEG versions for browsers without WebP support
    for (const size of imageSizes) {
      const jpegOutput = path.join(
        DEST_DIR,
        `${filenameWithoutExt}-${size}.jpg`,
      );
      await image
        .clone()
        .resize({ width: size, withoutEnlargement: true })
      ...
      console.log(
        `  ✓ Generated JPEG (${size}px): ${path.basename(jpegOutput)}`,
      );
    }
    ...
    await image.clone().webp({ quality: WEBP_QUALITY }).toFile(fullWebpOutput);

    console.log(
      `  ✓ Generated full-size WebP: ${path.basename(fullWebpOutput)}`,
    );
    ...
    console.log(
      `  🔍 Size reduction: ${originalSize / 1024} KB → ${webpSize / 1024} KB (${savings}% smaller)`,
    );
